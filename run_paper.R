...
# ---------------------------------------------------------------------------
# FIG 1: IRR under constant hazard
# ---------------------------------------------------------------------------
...
ggplot(data.frame(t=rep(t,2), IRR=c(IRR1, IRR2), lambda=rep(c(lambda1, lambda2), each=length(t))),
       aes(x=t, y=IRR, color=factor(lambda))) +
  geom_line(size=2) + theme_minimal() + theme(legend.position="top") +
  labs(x="Time", y="Incidence Rate Ratio") +
  scale_color_brewer("Lambda", palette="Dark2") + theme(text=element_text(size=24))

ggsave("article/figures/IRR.png", width=10, height=6, bg="white")


# ---------------------------------------------------------------------------
# FIG: Frailty / heterogeneity
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Marginal vs average VE — 4-group NCD model
# ---------------------------------------------------------------------------
...
df <- data.frame(t=res[, "t"],
                 MarginalRatio=(exp[, "C[2]"] + exp[, "C[4]"] / (exp[, "C[1]"] + exp[, "C[3]"])),
                 AverageRatio=(exp[, "C[2]"] / exp[, "C[1]"] + exp[, "C[4]"] / exp[, "C[3]"]))
...
for(sd in c(0.01, 0.4)){
  ...
  print(beta)
  for(f in c(0.1, 0.7)){
    res[[length(res) + 1]] <- get_eate_frailty(0.5, sd=sd, f=f, beta=beta, N=1000, t=100, n_frailty=25) %>%
      mutate(sd=sd, f=glue::glue("{round(f*100,0)}% vaccinated"))
  }
}
res <- rbindlist(res)
...
# ---------------------------------------------------------------------------
# EATE: different coverage and beta
# ---------------------------------------------------------------------------
...
non      <- def_run_4_4_nl(0.5, 0.5, lambda_h, lambda_l, alpha, M=m_random)
conf     <- def_run_4_4_nl(0.5, 0.9, lambda_h, lambda_l, alpha, M=m_random)
non_sep  <- def_run_4_4_nl(0.5, 0.5, lambda_h, lambda_l, alpha, M=m_sep)
conf_sep <- def_run_4_4_nl(0.5, 0.9, lambda_h, lambda_l, alpha, M=m_sep)

plot_df <- rbind(
  non$rr      %>% select(t, rr=CRR_t) %>% mutate(conf="No",         mixing="Random mixing"),
  strat_est(conf, frac_h=0.5)          %>% mutate(conf="Controlled", mixing="Random mixing"),
  conf$rr     %>% select(t, rr=CRR_t) %>% mutate(conf="Yes",        mixing="Random mixing"),
  non_sep$rr  %>% select(t, rr=CRR_t) %>% mutate(conf="No",         mixing="Assortative mixing"),
  strat_est(conf_sep, frac_h=0.5)      %>% mutate(conf="Controlled", mixing="Assortative mixing"),
...
# ---------------------------------------------------------------------------
# SSA: exploratory linear vs non-linear
# ---------------------------------------------------------------------------

init.values = c(S=10^3)
transitions  = list(c(S=-1))

init.values.SIR = c(S=10^3, I=1, R=0)
transitions.SIR  = list(c(S=-1, I=1), c(I=-1, R=1))
...
r_non <- list()
for(i in 1:1000){
  r <- ssa.adaptivetau(init.values, transitions, DecayRateF, params, tf=200, tl.params=list(epsilon=0.001))
  r_lin[[i]] <- as.data.frame(r) %>% mutate(sim=i)
  r <- ssa.adaptivetau(init.values.SIR, transitions.SIR, SIRRateF, params_nl, tf=50, tl.params=list(epsilon=0.001))
...
res_lin[, cum:=10^3 - S]
res_non[, cum:=10^3 - S]
comb <- rbind(res_lin %>% mutate(type="Linear") %>% select(time, cum, type, sim),
              res_non %>% mutate(type="Non-Linear") %>% select(time, cum, type, sim))
comb[, int_time:=ceiling(time)]

ggplot(comb) + geom_line(aes(x=time, y=cum, group=sim)) + facet_wrap(.~type, scales="free_x") + theme_minimal() + scale_color_brewer("Groups", palette="Dark2") + xlab("Time") + ylab("Cumulative incidence") + theme(text=element_text(size=20))
...
for(i in c(2,3,4)){
  ...
  res[[length(res)+1]]      <- data.frame(N=50*i, pos_vac=10*i, pos_cont=20*i, med=ci_ea[2], eate_l=ci_ea[1], eate_u=ci_ea[3], type="Full population", sto_type="Individual")
  ...
  ci_ea <- quantile(EATEs$EATEs_ind, probs=c(0.025, 0.5, 0.975))
  res[[length(res)+1]]      <- data.frame(N=50*i, pos_vac=10*i, pos_cont=20*i, med=ci_ea[2], eate_l=ci_ea[1], eate_u=ci_ea[3], type="Linear", sto_type="Individual")
...
ggplot(res) + geom_line(aes(x=N, y=width, col=type), size=1.2) + geom_point(aes(x=N, y=width, col=type), size=3) + theme_minimal() + labs(y="Width of 95% CI", x="Sample size") + theme(text=element_text(size=20)) + scale_color_brewer("Method", palette="Dark2")
...
ci_EATEs <- quantile(EATEs$EATEs_stacked, probs=c(0.025, 0.5, 0.975))
ggplot(data.frame(EATEs=EATEs$EATEs_stacked) %>% filter(EATEs<10)) + geom_histogram(aes(x=EATEs), bins=30) + theme_minimal() + labs(y="Frequency", x="EATE") + theme(text=element_text(size=20)) + geom_vline(xintercept=pos_vac/pos_cont, col="red") + annotate("text", x=4, y=550, label=glue::glue("EATE={round(ci_EATEs[2],2)}, 95% CI=({round(ci_EATEs[1],2)}, {round(ci_EATEs[3],2)})"), color="blue", hjust=0, size=7) + theme(text=element_text(size=20))
