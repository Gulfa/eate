...
# ---------------------------------------------------------------------------
# Rate functions for adaptivetau simulations
# ---------------------------------------------------------------------------

# Single-compartment decay: used for exploratory SSA demos
DecayRateF <- function(x, p, t) {
  return(c(x["S"] * p$rate))
}

# Single-group SIR: used for exploratory SSA demos
SIRRateF <- function(x, p, t) {
  return(c(p$beta*x["S"]*x["I"]/p$N, x["I"]))
}

# Two-group SIR rate function (used by calc_EATEs / compare_fixed_vals)
NonLinearRateF <- function(x, p, t) {
  return(c(p$beta*x["S1"]*(x["I1"] + x["I2"])/p$N, x["I1"],
           p$alpha*p$beta*x["S2"]*(x["I1"] + x["I2"])/p$N, x["I2"]))
}

# Two-group SIR with embedded sub-population
NonLinearRateFEm <- function(x, p, t) {
  return(c(p$beta*x["S1"]*(x["I1"] + x["I2"] + x["I3"])/p$N, x["I1"],
           p$alpha*p$beta*x["S2"]*(x["I1"] + x["I2"] + x["I3"])/p$N, x["I2"],
           p$beta*x["S3"]*(x["I1"] + x["I2"] + x["I3"])/p$N, x["I3"]))
}

# Two-group linear rate function (used by calc_EATEs / compare_fixed_vals)
LinearRateF <- function(x, p, t) {
  return(c(p$beta*x["S1"], 0, p$alpha*p$beta*x["S2"], 0))
}

# ---------------------------------------------------------------------------
# EATE estimation via stochastic simulations
# ---------------------------------------------------------------------------

calc_EATEs <- function(N_vac, N_cont, pos_vac, pos_cont, I0_vac=0, I0_cont=1, t=20, n_sims=1000,
                       inner_n=10, cores=10, epsilon=0.05, frac_cont=1, rate_func=NonLinearRateF, scale_beta=1){

  init.vals <-
    c(S1=N_cont, I1=I0_cont, R1=0, C1=0, N1=N_cont,
      S2=N_vac,  I2=I0_vac,  R2=0, C2=0, N2=N_vac)
  transitions.vacSIR = list(
  ...
  if(frac_cont < 1){
    init.vals <-
      c(S1=N_cont*frac_cont, I1=I0_cont*frac_cont, R1=0, C1=0, N1=N_cont*frac_cont,
        S2=N_vac,             I2=I0_vac,             R2=0, C2=0, N2=N_vac,
        S3=N_cont*(1-frac_cont), I3=I0_cont*(1-frac_cont), R3=0, C3=0, N3=N_cont*(1-frac_cont))
    transitions.vacSIR = list(
      c(S1=-1, I1=1, C1=1), c(I1=-1, R1=1),
      c(S2=-1, I2=1, C2=1), c(I2=-1, R2=1),
      c(S3=-1, I3=1, C3=1), c(I3=-1, R3=1))
  }
  ...
  check_param <- function(i){
    ...
    if(abs((r[nrow(r), "C1"] - pos_cont))/(N_cont*frac_cont) < epsilon &
       abs((r[nrow(r), "C2"] - pos_vac))/(N_vac) < epsilon){
  ...
  eate_diff_p1 <- compare_cf(
    init.vals, init.vals2, transitions.vacSIR,
    param_list, param_list, rate_func, t,
    ((C2_Z + 0.5)/N2_Z) / ((C1_X + 0.5)/N1_X),
    N=length(param_list)*inner_n, n_cores=cores, paramlist=TRUE)
  print("finished 1")

  init.vals2 <- lapply(init.vals, function(tmp){
    tmp["S1"] <- tmp["S1"] + 1;  tmp["N1"] <- tmp["N1"] + 1
    tmp["S2"] <- tmp["S2"] - 1;  tmp["N2"] <- tmp["N2"] - 1
    return(tmp)
  })
  ...
  return(list(
    EATEs_stacked=(N_cont*frac_cont*sta_p1 + N_vac*sta_m1)/(N_cont*frac_cont + N_vac),
    EATEs_ind=(N_cont*frac_cont*ind_p1 + N_vac*ind_m1)/(N_cont*frac_cont + N_vac),
    betas=betas, alphas=alphas, I0s=I0s))
}


compare_fixed_vals <- function(param_list, N_vac, N_cont, I0_vac=0, I0_cont=1, t=20,
                                n_sims=1000, inner_n=10, cores=10, rate_func=NonLinearRateF, scale_beta=0){
  init.vals <-
    c(S1=N_cont, I1=I0_cont, R1=0, C1=0, N1=N_cont,
      S2=N_vac,  I2=I0_vac,  R2=0, C2=0, N2=N_vac)
  transitions.vacSIR = list(
  ...
  init.vals2 <- copy(init.vals)
  init.vals2["S1"] <- init.vals2["S1"] - 1;  init.vals2["N1"] <- init.vals2["N1"] - 1
  init.vals2["S2"] <- init.vals2["S2"] + 1;  init.vals2["N2"] <- init.vals2["N2"] + 1
  ...
  eate_diff_p1 <- compare_cf(
    ...
    param_list, param_list, rate_func, t,
    ((C2_Z + 0.5)/N2_Z) / ((C1_X + 0.5)/N1_X),
    N=length(param_list)*inner_n, n_cores=cores, paramlist=FALSE)
  print("finished 1")
  ...
  return(list(
    EATEs_stacked=(N_cont*sta_p1 + N_vac*sta_m1)/(N_cont + N_vac),
    EATEs_ind=(N_cont*ind_p1 + N_vac*ind_m1)/(N_cont + N_vac)))
}


compare_vac_cov <- function(cov1, cov2, rate_func, params, T=20, N=1000, I0=1, n_sim=100, cores=10){
  init.vals1 <-
    c(S1=round(N*(1-cov1)), I1=I0, R1=0, C1=0, N1=N,
      S2=round(N*cov1),     I2=0,  R2=0, C2=0, N2=N)
  init.vals2 <-
    c(S1=round(N*(1-cov2)), I1=I0, R1=0, C1=0, N1=N,
      S2=round(N*cov2),     I2=0,  R2=0, C2=0, N2=N)
  transitions.vacSIR = list(
    c(S1=-1, I1=1, C1=1), c(I1=-1, R1=1),
    c(S2=-1, I2=1, C2=1), c(I2=-1, R2=1))

  ind1 <- parallel::mclapply(1:n_sim, function(i)
    regularise(as.data.frame(ssa.adaptivetau(init.vals1, transitions.vacSIR, rate_func, params, tf=T)), seq(0, T, 0.1)) %>% mutate(sim=i),
    mc.cores=cores)
  ind2 <- parallel::mclapply(1:n_sim, function(i)
    regularise(as.data.frame(ssa.adaptivetau(init.vals2, transitions.vacSIR, rate_func, params, tf=T)), seq(0, T, 0.1)) %>% mutate(sim=i),
    mc.cores=cores)

  tot <- rbindlist(lapply(1:n_sim, function(i)
    data.frame(time=ind1[[i]]$time, tot1=ind1[[i]]$C1 + ind1[[i]]$C2,
               tot2=ind2[[i]]$C1 + ind2[[i]]$C2, sim=i) %>%
      mutate(inc1=tot1 - lag(tot1), inc2=tot2 - lag(tot2))))
  return(tot)
}
...
bootstrap_ci <- function(res, n=3000, conf=0.8){
  data <- data.frame(
    I=c(c(rep(1, res$T), rep(0, res$N_vac - res$T)),
        c(rep(1, res$C), rep(0, res$N_cont - res$C))),
